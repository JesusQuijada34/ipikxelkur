...
def safe_extract_zip(archive, destination):
    ...
    for info in archive.infolist():
        ...
        if os.path.commonpath((root, target)) != root:
            raise ValueError(f"Ruta fuera del destino: {info.filename}")
        ...
        try:
            ...
            if requests:
                ...
                if total and total > MAX_UPDATE_BYTES:
                    raise ValueError("La actualización supera el límite de 100 MB")
                with open(temp_zip, "wb") as f:
                    for chunk in r.iter_content(8192):
                        if not self._running: return
                        down += len(chunk)
                        if down > MAX_UPDATE_BYTES:
                            raise ValueError("La actualización supera el límite de 100 MB")
                        f.write(chunk)
    ...
    def _kill_target(self, target_name):
        ...
        try:
            if sys.platform == "win32":
                ...
                subprocess.call(["taskkill", "/F", "/IM", f"{safe_name}.exe"], check=False)
                subprocess.call(["taskkill", "/F", "/IM", safe_name], check=False)
            else:
                safe_name = os.path.basename(target_name)
                subprocess.call(["pkill", "-9", "-x", safe_name], check=False)
        except Exception as e:
            log(f"Kill error: {e}")
